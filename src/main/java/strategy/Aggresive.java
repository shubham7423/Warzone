...
//					}
					...
					List<Integer> l_strongNeighbors = getStrongNeighbors(l_strongCountry);							
					...
					if(l_strongCountry.getNumberOfArmiesPresent() - l_randomArmies < 2) {
						continue;
					}
					if(d_gameEngine.getGameMap().getCountries().get(l_strongNeighbors.get(l_randomEnemyCountry)).getNumberOfArmiesPresent() > l_strongCountry.getNumberOfArmiesPresent()) {
						continue;
					}
					
					l_order = new Advance(d_player, l_strongCountry.getId(), l_strongNeighbors.get(l_randomEnemyCountry), l_randomArmies);
					System.out.println("Advance from: " + l_strongCountry.getId() + ", to: " + l_strongNeighbors.get(l_randomEnemyCountry) + ", armies:  " + l_randomArmies);
					d_gameEngine.d_logEntryBuffer.setString("Advance from: " + l_strongCountry.getId() + ", to: " + l_strongNeighbors.get(l_randomEnemyCountry) + ", armies:  " + l_randomArmies);
					l_isComplete = true;
					break;
...
//					}
	...
	public List<Integer> getStrongNeighbors(Country l_strongCountry) {
		...
		for(Country l_country: l_strongCountry.getNeighborCountries()) {
			if(!l_country.getPlayer().equals(d_player)) {
